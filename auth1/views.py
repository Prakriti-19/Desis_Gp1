...
def donor_home(request):
    ...
    lives = user_donation.filter(
        ngo_status=False, donor_status=False).aggregate(Sum('quantity'))
...
def NgoLoginView(request):
    if request.method == 'POST':
        ...
        user = MyUserBackend.authenticate(
            request, username=username, password=password, backend='auth1.backends.MyUserBackend')
